...
function ViberBot(loggerOrConfiguration, configuration) {
	...
	let logger = loggerOrConfiguration || NoopLogger;

	if (!configuration && !_.isArray(configuration)) {
	...
	for (let config of configuration) {
		...
		this.authTokens.push(config.authToken);
	}
	...
	this._middleware = new Middleware(this._logger, new MessageValidator(this._logger, this.authTokens));
...
ViberBot.prototype.setWebhook = function(url, isInline, token) {

	if (token) {
		this._client.setWebhook(url, isInline, token);
	} else {

		for (let token of this.authTokens) {
			this._client.setWebhook(url, isInline, token);
		}
	}
};
...
ViberBot.prototype._setupConversationStartedHandler = function() {
	...
	this.on(EventConsts.CONVERSATION_STARTED, (response, isSubscribed, context) => {
		_.each(self._callbacks[EventConsts.CONVERSATION_STARTED], callback => {
			//new response.userProfile => response
			callback(response, isSubscribed, context, (responseMessage, optionalTrackingData) => {
...
ViberBot.prototype._checkConfiguration = function (config) {
	const missingFields = this._getMissingFieldsInConfiguration(config);
	if (!_.isEmpty(missingFields)) {
		throw new Error(`Invalid configuration ${config}. Missing fields: ${missingFields}`);
	}
}
...
ViberBot.prototype._handleEventReceived = function(data) {
	...
	switch (data.event) {
		case EventConsts.MESSAGE_RECEIVED: {
            ...
            message.token = data.message_token; 
			const response = new Response(this, userProfile, data.silent, data.reply_type, data.chat_id, data.authToken);
