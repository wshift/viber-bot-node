...
ViberClient.prototype.setWebhook = function(url, isInline, authToken) {
...
ViberClient.prototype._sendRequest = function(endpoint, data, authToken) {
	...
	const token = authToken || this._bot.authToken;
	const dataWithAuthToken = Object.assign({ "auth_token": token }, data);
	const options = {
		json: true, rejectUnauthorized: true, parse: true,
		user_agent: this._userAgent, // eslint-disable-line
		headers: {
			[VIBER_AUTH_TOKEN_HEADER]: token
		}
	};
